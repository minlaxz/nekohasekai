...
- ~~Should this repo drop `utls` from the client scaffold?~~ **Resolved 2026-09-20: keep it.** Both paths work for ShadowTLS v3: a `utls` config implements `WithSessionIDGenerator`, otherwise sing-box falls back to `shadowtls.DefaultTLSHandshakeFunc` ([`protocol/shadowtls/outbound.go:57-69`](https://github.com/SagerNet/sing-box/blob/v1.14.1/protocol/shadowtls/outbound.go#L57-L69)), which calls `utls.Client` with no browser hello, i.e. a Go-default ClientHello ([sing-shadowtls `tls_wrapper.go:23-45`](https://github.com/SagerNet/sing-shadowtls/blob/v0.2.1/tls_wrapper.go#L23-L45)). Dropping `utls` therefore only swaps a Chrome-like ClientHello for a Go-like one toward the borrowed `server_name` site, which is the more distinctive of the two for a camouflage protocol (**inferred**, not measured). Upstream's warning says the imitation is imperfect, not that it is harmful; no removal version is announced, and official release builds still carry `with_utls` ([`release/DEFAULT_BUILD_TAGS`](https://github.com/SagerNet/sing-box/blob/v1.14.1/release/DEFAULT_BUILD_TAGS)). Upstream's suggested alternative (NaiveProxy) is a protocol change, not a drop-in. Revisit if upstream schedules `utls` for removal. Still untested: a live handshake with `utls` disabled.
