...
- **Init** — the one-time step on first container start that seeds Server config and Client template into the persistent volume and fills in deploy-specific values. Seeding runs once (`.initialized`); ports, SNI, and the ShadowTLS password are written once (`.configured`); Public IP is refreshed on every start. Every other Client template file (everything but `outbounds.json`) is re-copied from the image (`/defaults`) on every start, so `pull` + `up -d` is the whole upgrade path.
