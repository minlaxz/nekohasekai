...
- **Template refresh** — the every-start step that copies every Client template file except `outbounds.json` from the image (`/defaults`) into the volume. Makes `pull` + `up -d` the whole upgrade path; `outbounds.json` is skipped because it holds Deploy values.
...
- **User rule set** — a sing-box rule-set source file the user hosts and passes by URL when fetching their Profile config. Only domain suffixes are accepted. Fetched and checked on every request; its domain suffixes join the Profile config's proxied-in-Partial-mode list, alongside the admin-curated rule sets (`my-rules`, geosite). Optional, one per request. A bad or unreachable file fails the request rather than being dropped silently.
